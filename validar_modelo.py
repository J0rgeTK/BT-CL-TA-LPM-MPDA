...
def ejecutar_validacion() -> pd.DataFrame:
    ...
    # 3. Insumos OD procesados: la ejecución no debe depender de Excel binarios.
    faltantes_procesados = [p for p in ODH.PROCESSED_FILES.values() if not p.exists()]
    rows.append(_ok(
        "Insumos OD procesados CSV",
        len(faltantes_procesados) == 0,
        "Faltantes: " + (", ".join(p.name for p in faltantes_procesados) if faltantes_procesados else "ninguno"),
    ))

    # 4. Ejecución del motor mensual.
    ...
    # 5. Consistencia de totales mensuales/anuales entre detalle y resumen.
    ...
    # 6. Sensibilidad mensual: cambio en marzo L2 debe afectar marzo y no todos los meses.
    ...
    # 7. Feriados: Biotren sin operación, Laja opera como fin de semana.
    ...
    # 8. OD híbrido: ejecución y consistencia de totales.
    ...
    # 9. Matrices por tipo: orden, dimensiones e ingresos.
    ...
    # 10. Tarifas e ingresos para pares con viajes proyectados.
    ...
    # 11. Validación explícita de arreglos NumPy/Pandas no escribibles en el balance OD.
